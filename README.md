...
Claude 拉到的数据存在严重的**新鲜度**和**准确性**问题——WebSearch 给的是"能搜到的最新一篇报道"，不是"当前实时状态"；媒体报道的数字可能摘错、口径不一；Claude 自己估算时可能用错 EPS 或净资产基数。

**每份分析文档都会标注数据可信度**：

| Emoji | 含义 | 来源 |
|-------|------|------|
| 🟢 | 官方数据 | 年报/季报/公司公告/交易所披露/tdx MCP |
| 🟡 | 第三方研报 | 主流券商、知名研究机构 |
| 🟠 | 媒体报道 | WebSearch、财经媒体、行业公众号 |
| 🔴 | Claude 自算 | 估值二级指标，展示公式 + 输入来源 |
| ⚠️ | 存疑/过期 | 数据日期 >3 个月、口径不明、来源单一 |

**估值数据三铁律**：
1. PE / PB / ROE 等二级指标**必须自算**，不引用搜索结果。
2. **展示公式 + 输入来源**，让用户能复核。
3. **标注口径**（TTM vs 年化、平均 vs 期末、整体 vs 分部）。

**每份文档都有"数据缺口"章节**，明确告诉用户哪些数据需要手动补充：
- 最新卖方研报（30 天内）
- 管理层最新表态（业绩说明会/路演纪要）
- 渠道/草根调研数据
- 行业专家访谈
